...
""".stripIndent()
    ...
    private String computeContainerName() { "${PipelineDefaults.CONTAINER_NAME_PREFIX}-${steps.env.BUILD_NUMBER ?: '0'}-destroy" }
    private String computeImageName() { "rancher-destroy-${steps.env.BUILD_NUMBER ?: '0'}" }
    private String computeVolumeName() { "DestroySharedVolume-${steps.env.BUILD_NUMBER ?: '0'}" }
...
"""
    ...
    private void logInfo(String msg) { steps.echo "${PipelineDefaults.LOG_PREFIX_INFO} ${timestamp()} ${msg}" }
    ...
    private static String timestamp() { new Date().format('yyyy-MM-dd HH:mm:ss') }
